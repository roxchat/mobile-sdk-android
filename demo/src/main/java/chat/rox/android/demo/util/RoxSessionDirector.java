...
import chat.rox.android.demo.SettingsFragment;
...
public class RoxSessionDirector {
    ...
    public static void createSessionBuilderWithAnonymousVisitor(Context context, Rox.PushSystem pushSystem, OnSessionBuilderCreatedListener listener) {
        Rox.SessionBuilder sessionBuilder = newSessionBuilder(context);
        retrieveToken(context, sessionBuilder, listener);
    }

    public static void createSessionBuilderWithAuth1Visitor(Context context, Rox.PushSystem pushSystem, OnSessionBuilderCreatedListener listener) {
    ...
    public static void createSessionBuilderWithAuth2Visitor(Context context, OnSessionBuilderCreatedListener listener) {
        ...
        if (visitorsData == null) {
            Log.e("Rox Demo", "Visitors fields not exists");
            listener.onError(SessionBuilderError.AUTH_USER_ERROR);
            return;
        }
        Rox.SessionBuilder sessionBuilder = newSessionBuilder(context).setVisitorFieldsJson(visitorsData);
        retrieveToken(context, sessionBuilder, listener);
    }

    private static void retrieveToken(
        Context context,
        Rox.SessionBuilder builder,
        ...
        Rox.PushSystem pushSystem = Rox.PushSystem.NONE;
    ...
    private static void retrieveFirebaseToken(Context context, Rox.SessionBuilder builder, OnSessionBuilderCreatedListener listener) {
        FirebaseMessaging.getInstance().getToken().addOnCompleteListener(task -> {
            ...
            builder.setPushSystem(Rox.PushSystem.FCM);
            listener.onSessionBuilderCreated(builder);
        });
    }

    private static void retrieveHuaweiToken(Context context, Rox.SessionBuilder builder, OnSessionBuilderCreatedListener listener) {
        WorkingQueue.getInstance().sendTaskAndListenResultOnMainThread(
            new WorkingQueue.RequestCallback<String>() {
                ...
                public void onResult(String result) {
                    ...
                    builder.setPushSystem(Rox.PushSystem.HPK);
                    listener.onSessionBuilderCreated(builder);
                }
    ...
    private static Rox.SessionBuilder newSessionBuilder(Context context) {
        ...
        if (sharedPref.getBoolean(KEY_FILE_LOGGER, false)) {
            try {
                roxLog = FileLogger.getAppSpecificDirLogger(context, "Rox.txt");
    ...
    private static String getJsonOfAuthVisitor(String fields, String expires, String privateKey) throws Exception {
        final String LOG_TAG = "ROX_AUTH_VISITOR";
